...
//
// runTest should be a function of type func(*testing.T, <TestType>), where TestType
// is the type of the test contained in test files.
...
func (tm *testMatcher) runTestFile(t *testing.T, path string, runTest interface{}) {
	if r, _ := tm.findSkip(trimTestName(t.Name())); r != "" {
		t.Skip(r)
	}
	t.Parallel()

	// Check type of runTest.
	stringT := reflect.TypeOf("")
	testingT := reflect.TypeOf((*testing.T)(nil))
	ftyp := reflect.TypeOf(runTest)
	if ftyp.Kind() != reflect.Func || ftyp.NumIn() != 2 || ftyp.NumOut() != 0 || ftyp.In(0) != testingT {
		panic(fmt.Sprintf("bad test function type: want func(*testing.T, <TestType>), have %s", ftyp))
	}
	testType := ftyp.In(1)

	// Load the file as map[string]<testType>.
	mp := reflect.New(reflect.MapOf(stringT, testType))
	if err := readJsonFile(path, mp.Interface()); err != nil {
		t.Fatal(err)
	}
	...
	if len(keys) == 1 {
		runTestFunc(runTest, t, m, keys[0])
	} else {
...
func runTestFunc(runTest interface{}, t *testing.T, m reflect.Value, key string) {
	reflect.ValueOf(runTest).Call([]reflect.Value{reflect.ValueOf(t), m.MapIndex(reflect.ValueOf(key))})
}

func trimTestName(s string) string {
